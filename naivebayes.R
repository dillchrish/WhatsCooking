...
# -----------------------------
# Recipe (same idea as yours)
# -----------------------------
whats_recipe <- recipe(cuisine ~ ., data = train_model_df) %>%
  step_zv(all_predictors()) %>%        # remove zero-variance columns
  step_normalize(all_predictors())     # optional, fine to keep

# -----------------------------
# Naive Bayes model
# -----------------------------
nb_mod <- naive_Bayes(
  smoothness = 1,     # Laplace smoothing (try 0.5, 1, 2 if you want)
  Laplace    = 1
) %>%
...
# Predict + submission
# -----------------------------
test_preds <- predict(nb_fit, new_data = test_model_df)
...
write_csv(
  submission,
  "~/Downloads/whatscooking_tfidf_nb.csv"
)
