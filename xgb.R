...
whats_recipe <- recipe(cuisine ~ ., data = train_model_df) %>%
  ...
  step_normalize(all_predictors()) # remove zero-variance columns

n_pred <- ncol(train_model_df) - 1

xgb_mod <- boost_tree(
  trees          = 500,                           # was 400
  tree_depth     = 6,                             # a bit shallower than 7
  learn_rate     = 0.07,                          # a bit faster than 0.05
  min_n          = 4,                             # slightly more flexible
  loss_reduction = 0.01,                          # small regularization (gamma)
  sample_size    = 0.7,                           # a bit more randomness
  mtry           = floor(0.4 * n_pred)            # use ~40% of features per split
) %>%
...
test_preds <- predict(xgb_fit, new_data = test_model_df)
...
write_csv(
  submission,
  "~/Downloads/whatscooking_tfidf_xgb_tweaked.csv"
)